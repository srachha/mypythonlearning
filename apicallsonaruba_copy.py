...
class api_session:
  def __init__(self, host, username, password, port=4343, SSL=True, check_ssl=True, verbose=False, retrys=3, retry_wait=0.5):
    if SSL:
      protocol = "https"
    else:
      protocol = "http"
    ...
    self.session = None
    self.api_token = None
    self.api_url = "{}://{}:{}/v1/".format(protocol,host,port)
